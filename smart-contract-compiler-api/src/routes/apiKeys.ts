...
router.post('/', requireMasterKey, async (req, res) => {
  try {
    ...
    const validPermissions = ['compile', 'deploy', 'gas-estimate', 'contract-info', 'verify']
    ...
    const result = await apiKeyManager.createApiKey(name.trim(), userId.trim(), apiPermissions)
    res.status(201).json({
      success: true,
      ...
      usage: {
        ...
        example: `curl -H "Authorization: Bearer ${result.key}" https://api.contractforge.io/compile`
      }
    })
  } catch (error: any) {
